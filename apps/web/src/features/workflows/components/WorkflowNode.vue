...
<template>
  <div class="graph-node" :class="[data.node.type, { selected }]" data-testid="workflow-node" :data-node-id="data.node.id">
    ...
    <div class="node-topline"><span class="node-icon" aria-hidden="true">{{ data.node.type === 'start' ? '↗' : data.node.type === 'end' ? '■' : '✦' }}</span><span class="node-type">{{ nodeLabels[data.node.type] }}</span><span class="node-type-version">v1</span></div>
