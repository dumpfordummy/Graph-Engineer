...
import RunControl from '../runs/RunControl.vue'
...
<template>
  <div class="editor-page">
    <header class="editor-header"><RouterLink to="/" class="back-link" aria-label="Back to workflows">← <span>Workflows</span></RouterLink><span class="header-divider"></span><span class="brand-small">Graph Engineering</span><RouterLink to="/settings/connections" class="back-link">Model connections</RouterLink><nav class="workflow-view-tabs" aria-label="Workflow views"><span aria-current="page">Design</span><RouterLink :to="store.persisted && store.document ? `/runs?workflowId=${store.document.workflow.id}` : '/runs'" class="back-link">Runs</RouterLink></nav><span class="local-badge"><span></span> Local workspace</span></header>
    <div v-if="store.loading" class="loading-state" role="status">Loading workflow…</div>
    <template v-else-if="store.document">
      <div class="editor-toolbar"><div class="workflow-title"><label class="sr-only" for="workflow-name">Workflow name</label><input id="workflow-name" :value="store.document.workflow.name" aria-label="Workflow name" maxlength="120" @input="store.setMetadata('name', ($event.target as HTMLInputElement).value)"><span data-testid="save-status" role="status" class="save-status" :class="{ unsaved: store.dirty }"><span></span>{{ store.status }}</span></div><div class="toolbar-actions"><button :disabled="store.saving || store.importing" @click="importFile?.click()">Import JSON</button><button @click="exportJson">Export JSON</button><button :disabled="store.validating" @click="store.validate">{{ store.validating ? 'Validating…' : 'Validate' }}</button><button class="primary" :disabled="store.saving || store.importing || (!store.dirty && store.persisted)" @click="save">{{ store.saving ? 'Saving…' : 'Save' }}</button><RunControl :key="store.document.workflow.id" /></div><input ref="importFile" data-testid="import-file" type="file" accept=".json,application/json" hidden @change="importJson"></div>
