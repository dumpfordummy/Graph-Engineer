...
Validation report: `{ "structurallyValid": true, "valid": false, "issues": [{ "code": "missing_start", "severity": "error", "message": "Add exactly one Start node.", "path": "definition.nodes", "nodeId": "optional", "edgeId": "optional" }], "scope": "Structure and draft configuration only. Provider configuration and execution readiness are not checked by graph validation. Execution is unavailable until M3." }`. `valid` includes semantic checks. Issues use optional node/edge references to focus the editor.
...
- DELETE /api/providers/{id} takes JSON { expectedRevision }; returns 204; stale or saved-workflow references = 409 with safe conflict detail (no graph rewrite).
